...
- Social publishing now has a controlled Meta delivery path and `/publishing` queue for approved schedule entries; publishing previews and automatic reconciliation remain outstanding. OAuth exchanges callback credentials for long-lived Meta authorization and shows operators when reconnection is approaching.
...
- The content workspace now supports persistent light and dark modes, defaults to the device preference for first-time users, and keeps the full Ixara logo on a contrast-safe black field.
